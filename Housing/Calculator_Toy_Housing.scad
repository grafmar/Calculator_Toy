...
color("orange")OnOffSwitch(inletHeight); // for inlet
color("black")OnOffSwitchInlet(inletHeight); // the inlet
color("orange")Keypad(inletHeight); // for inlet
color("black")KeypadInlet(inletHeight); // the inlet

Calculator_Toy_PCB();
//color("orange")OnOffSwitch(); // embossed
//color("orange")Keypad(); // embossed
color("silver")Housing();
color("silver")BottomSlide_compl();
color("silver")ClosingLid();

module ClosingLid() {
    translate([0,-thick-1.5,-5-1])rotate([0,-90,-90])difference(){
        union(){
            cube([17,pcbW,1]);
            cube([1,pcbW,4]);
        }
        for(i=[-1,1])translate([16/2+1,pcbW/2+i*(pcbW/2-10),0])cylinder(d=1.5,h=3,center=true,$fs=.5);
    }
}
...
difference(){
    ...
    // Screw holes
    translate([0,-thick-1.5,-5-1])rotate([0,-90,-90])for(i=[-1,1])translate([16/2+1,pcbW/2+i*(pcbW/2-10),0])cylinder(d=1.5,h=30,center=true,$fs=.5);

}
}

module OnOffSwitch(embossDepth=1) {
    // On/Off-Switch
    color("#DDDDDD")translate([(2+21)*2.54,(3+9)*2.54,0]){
        ...
        difference() {
            translate([-5+clearance,-5+clearance,7+2+0.5])cube([10-2*clearance,10-2*clearance,14+thick+1-(7+2+0.5)]);
            translate([-3/2-clearance,-2/2-clearance,0])cube([3+2*clearance,2+2*clearance,12.5]);
            // PowerSymbol
            translate([0,0,14+thick+1 - embossDepth]){
                hull(){
...
module OnOffSwitchInlet(embossDepth) {
    // On/Off-Switch
    translate([(2+21)*2.54,(3+9)*2.54,0.01]){
        union() {
            // PowerSymbol
            translate([0,0,14+thick+1 - embossDepth]){
                hull(){
                    translate([0,-1.5,0])cylinder(d=1,h=embossDepth,$fs=0.3);
                    translate([0,1.5,0])cylinder(d=1,h=embossDepth,$fs=0.3);
                }
                difference() {
                    $fs=.5;
                    cylinder(d=7.5,h=embossDepth);
                    translate([0,0,-embossDepth])cylinder(d=5.5,h=3*embossDepth);
                }
            }
        }
    }
...
module Keypad(embossDepth=1) {
    // Keypad
    for(pos=buttonPos)translate([(2+pos.x-2)*2.54,(3+pos.y-2)*2.54,5]){
        ...
        difference() {
            ...
            translate([2*2.54,2*2.54,14+thick+1-5-embossDepth])linear_extrude(height=10,center=false,convexity=10)text(pos[2],size=7, halign="center",valign="center");
        }
    }
}


module KeypadInlet(embossDepth) {
    // Keypad
    for(pos=buttonPos)translate([(2+pos.x-2)*2.54,(3+pos.y-2)*2.54,5]){
        translate([2*2.54,2*2.54,14+thick+1-5-embossDepth])linear_extrude(height=embossDepth,center=false,convexity=10)text(pos[2],size=7, halign="center",valign="center");
    }
}
